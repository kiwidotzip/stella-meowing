...
register("command", () => {
  ...
  stands.forEach((stand) => {
    if (!(stand.getName() == "Armor Stand")) {
      ChatLib.chat(
        `${stand.getName()}&r at &dX: ${stand.getX()} Y: ${stand.getY()} Z: ${stand.getZ()}`
      );
    }
  });
...
register("step", () => {
  ...
  stands.forEach((stand) => {
    if (
      stand.getName().includes("Device") ||
      stand.getName().includes("Terminal") ||
      stand.getName().includes("Not Activated")
    ) {
