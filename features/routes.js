...
function addPoint(realpos, type) {
    ...
    if (type === "etherwarp") {
        for (var i = 0; i < recordingData.etherwarps.length; i++) {
            let [x, y, z] = [
                recordingData.etherwarps[i][0],
                recordingData.etherwarps[i][1],
                recordingData.etherwarps[i][2],
            ];
    ...
    if (type === "tnt") {
        for (var i = 0; i < recordingData.tnts.length; i++) {
            let [x, y, z] = [
                recordingData.tnts[i][0],
                recordingData.tnts[i][1],
                recordingData.tnts[i][2],
            ];
            let distance = calcDistance(pos, [x, y, z]);

            if (distance < 5) return;
        }
        recordingData.tnts.push([
            Math.round(pos[0]),
            Math.round(pos[1]),
            Math.round(pos[2]),
        ]);
    ...
    if (type === "mine") {
        for (var i = 0; i < recordingData.mines.length; i++) {
            let [x, y, z] = [
                recordingData.mines[i][0],
                recordingData.mines[i][1],
                recordingData.mines[i][2],
            ];
            if (x === pos[0] && y === pos[1] && z === pos[2]) return;
        }
    ...
    if (type === "interact") {
        for (var i = 0; i < recordingData.interacts.length; i++) {
            let [x, y, z] = [
                recordingData.interacts[i][0],
                recordingData.interacts[i][1],
                recordingData.interacts[i][2],
            ];
            if (x === pos[0] && y === pos[1] && z === pos[2]) return;
        }
...
function saveRoute(force) {
    ...
    if (!force) {
        for (var i = 0; i < routeData.length; i++) {
            if (routeData[i] === roomRID) {
                ...
                let yes = new TextComponent("&eOverride? &l&aYes").setClick(
                    "run_command",
                    "/route route_save_force"
                );
                yes.chat();
                let no = new TextComponent("&eOverride? &l&cNo").setClick(
                    "run_command",
                    "/route stop"
                );
                no.chat();
    ...
    FileLib.write(
        "eclipseAddons",
        "data/dungeons/routes/routes.json",
...
register("soundPlay", (pos, name) => {
    ...
    if (name === "random.explode") {
        ...
        if (!boom.some((i) => Player?.getHeldItem()?.getName()?.includes(i)))
            return;
...
register("command", (...args) => {
    if (args[0] === "next") step++;
        ...
        ChatLib.chat(
            "&6/srdb route &7Displays current secret route information!"
        );
        ...
        ChatLib.chat(
            "&cUnknown command. &7Try &6/srdb help &7for a list of commands"
        );
    }
}).setName("srdb");
...
register("command", (...args) => {
    if (args[0] === "record") {
        ...
        ChatLib.chat(
            "&cUnknown command. &7Try &6/route help &7for a list of commands"
        );
    }
}).setName("route");
