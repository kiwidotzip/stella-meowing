...
const chromaShader = new Shader(
    FileLib.read("eclipseAddons", "shaders/chroma/chroma3D.frag"),
    FileLib.read("eclipseAddons", "shaders/chroma/chroma3D.vert")
);
...
register("renderWorld", () => {
    ...
    if (settings().chromaHighlight) {
        if (settings().oldChroma) {
            ...
            chromaShader.uniform1f(
                "chromaSize",
                (30 * Client.getMinecraft().field_71443_c) / 1000
            );
            chromaShader.uniform1f(
                "timeOffset",
                (totalTicks + Tessellator.partialTicks) *
                    (6 / 360) *
                    settings().chromaOverlaySpeed
            );
    ...
    if (block?.type?.getRegistryName()?.toString()?.includes("stairs")) {
        ...
        if (fill) {
            if (settings().chromaHighlight && !settings().oldChroma)
                chromaShader.uniform1f("alpha", fa / 255);
